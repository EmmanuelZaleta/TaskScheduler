...
namespace YCC.AnalisisCogisInventario.Services.Sql
{
    ...
    {
        // Orden cl�sico fallback (si no detecta encabezado)
        ...
        public int Upload(string filePath, AppConfig cfg)
        {
            ...
            {
                ConsoleLogger.Warn("No se detect� encabezado; usando orden cl�sico COGI (15/16).");
                if (!TryReadDataWithClassic(filePath, out headerCols, out dataLines))
                {
                    ConsoleLogger.Warn("Archivo sin filas v�lidas.");
                    return 0;
                }
            }
            ...
            if (rows == 0)
            {
                ConsoleLogger.Warn("Archivo sin filas v�lidas para cargar.");
                return 0;
            }
            ...
            // copiar s�lo las columnas mapeadas
        ...
        private static List<string> NormalizeHeaderKeepOriginal(List<string> cells)
        {
            ...
            foreach (var c in cells)
            {
                ...
                if (t.Length == 0) continue;      // descarta vac�os
                list.Add(t);
            }
            return list;
        }
        ...
        private static void SafeEnsureTableWithColumns(SqlConnection cn, string tableName, IReadOnlyList<string> cols)
        {
            ...
            {
                ConsoleLogger.Warn($"Ensure columnas fall� en {tableName}: {ex.Message}");
...
@"SELECT c.name
 ...
 WHERE c.object_id = OBJECT_ID(@t)";
        ...
        {
            // [A] -> [A]; maneja corchetes y deja cualquier car�cter dentro
